...
| `direction` | `keyPressed` 对 `escape` 有单独的 reset 分支；`left/right/up/down` 会进入移动分支；其他值走 legacy no-op | 1. 重置方向：`escape` 2. 合法移动方向：`left/right/up/down` 3. 非法方向：任意其他字符串，如 `legacy` |
...
1. 代码路径覆盖：懒创建、合法移动但不合并、普通合并、`escape` reset、非法方向 no-op、胜利、失败都已覆盖
2. 输入取值覆盖：`id` 的两类取值、`direction` 的 reset/合法移动/非法三类取值、棋盘的可移动/不可移动两类取值都已覆盖
3. 判断逻辑覆盖：
